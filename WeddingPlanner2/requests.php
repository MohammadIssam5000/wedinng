																	...
																	<div class="elementor-element elementor-element-136d7da elementor-widget elementor-widget-vt-ceremony_section_title">
																		<div class="elementor-widget-container">
																			<div class="section-title-wrap  crmny-sectTitleElementor ">
																				<h2 class="section-title mt-5">Our Requests</h2>
																			</div>
																		</div>
																	</div>
																	<?php
																	$req="SELECT * FROM requests WHERE ownerEmail='$email' AND status != 'Declined' ORDER BY id DESC";
																	$reques=mysql_query($req, $db);
																	if(mysql_num_rows($reques) == 0) {?>
																	<br><br>
																		<div class="elementor-element elementor-element-136d7da elementor-widget elementor-widget-vt-ceremony_section_title" data-id="136d7da" data-element_type="widget" data-widget_type="vt-ceremony_section_title.default">
																			<p class="cart-empty woocommerce-info">There are no requests yet.</p>
																		</div>
																		<br><br><br><br><br><br>
																	<?php
																	} else {?>
																		...
																		<?php
																		while($request=mysql_fetch_array($reques)) {
																			if($request[3] == "Pending") {
																				$cancel="d-block";
																				$status="btn-info";
																			} else if($request[3] == "Acceptable") {
																				$cancel="d-none";
																				$status="btn-success";
																			} else if($request[3] == "Declined") {
																				$cancel="d-none";
																				$status="btn-danger";
																			}
																			$prod="SELECT * FROM products WHERE id='$request[1]'";
																			$produ=mysql_query($prod, $db);
																			$product=mysql_fetch_array($produ);?>
																			<li class="col-md-3 product type-product post-534 status-publish first instock product_cat-clothing product_cat-hoodies has-post-thumbnail shipping-taxable purchasable product-type-simple">
																				...
																				<div class="d-block">
																					<h4 class="mb-1">Applicant info</h4>
																					<?php
																					$Applicant="SELECT name, phone FROM customers WHERE email = '$request[2]'";
																					$ApplicantIn=mysql_query($Applicant, $db);
																					$ApplicantInfo=mysql_fetch_array($ApplicantIn);?>
																					<span class="applicant-info"><i class="fa fa-calendar"></i> Wedding Date: <?php echo $request[4];?></span>
																					<span class="applicant-info"><i class="fa fa-envelope"></i> <?php echo $request[2];?></span>
																					<span class="applicant-info"><i class="fa fa-user"></i> <?php echo $ApplicantInfo[0];?></span>
																					<span class="applicant-info"><i class="fa fa-phone"></i> <?php echo $ApplicantInfo[1];?></span>
																				</div>
																				<div class="btn <?php echo $status;?>"><?php echo $request[3];?></div>
																				<hr>
																				<div class="mt-3 <?php echo $cancel;?>">
																					<a href="accept.php?id=<?php echo $request[0];?>" class="btn btn-primary">Accept</a>
																					<a href="decline.php?id=<?php echo $request[0];?>&img=<?php echo $product[1];?>" class="btn btn-danger">Decline</a>
																				<div>
																			</li>
																		<?php
																		}?>
